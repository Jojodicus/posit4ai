...
//
// QACC state for FPU mode lives here (acc_q register).
// QACC state for PAU mode lives inside pau_top (internal quire register).
...
(
  ...
  // Used for QACC_ADD (PAU mode): QACC_ADD(a) = QMADD(a, 1.0)
  ...
  // ── Result and FPU accumulator registers ─────────────────────────────────────
  ...
  // Combinatorial result (valid in IDLE when valid_i is asserted)
  ...
  // Use un-registered inputs: pau_top samples its fu_data in the same cycle
  // pau_valid_i is asserted (first cycle of IDLE→WAIT transition).
  logic [DATA_WIDTH-1:0] pau_op_a, pau_op_b;
  fu_op                  pau_fu_op;

  always_comb begin
    pau_op_a  = operand_a_i;  // un-registered current inputs
    pau_op_b  = operand_b_i;
    pau_fu_op = PADD;  // don't-care default
    case (opcode_i)
      ...
      OP_QACC_ADD:   begin pau_fu_op = QMADD; pau_op_b = POSIT_ONE; end
      OP_QACC_MADD:  pau_fu_op = QMADD;
      OP_QACC_MSUB:  pau_fu_op = QMSUB;
