...
def load_pt(path: str):
    """
    Load float32 TorchScript (or state-dict) checkpoint and convert to
    posit<32,2>.  Returns (fc1_w, fc1_b, fc2_w, fc2_b) as uint32 arrays.
    """
...
class InferenceClient(Gtk.Window):
    ...
    def _build_ui(self):
        ...
        self._url_entry.set_width_chars(28)
        bar.pack_start(self._url_entry, False, False, 0)
        ...
        root.pack_start(img_box, False, False, 0)

        cam_frame = Gtk.Frame(label="Webcam")
        self._cam_img = Gtk.Image()
        self._cam_img.set_size_request(PREVIEW_W, PREVIEW_H)
        cam_frame.add(self._cam_img)
        img_box.pack_start(cam_frame, False, False, 0)

        proc_frame = Gtk.Frame(label="What model sees")
        self._proc_img = Gtk.Image()
        self._proc_img.set_size_request(28 * SMALL_SCALE, 28 * SMALL_SCALE)
        proc_frame.add(self._proc_img)
        img_box.pack_start(proc_frame, False, False, 0)

        # -- info row -----------------------------------------------------
        info = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=16)
        root.pack_start(info, False, False, 0)

        self._pred_lbl = Gtk.Label()
        self._pred_lbl.set_markup("<span size='xx-large'><b>-</b></span>")
        info.pack_start(self._pred_lbl, False, False, 0)

        self._stats_lbl = Gtk.Label(label="FPS: - | Latency: -")
        info.pack_start(self._stats_lbl, False, False, 0)
        ...
        ctrl.pack_start(self._start_btn, False, False, 0)
    ...
    def _on_upload_weights(self, _btn):
        ...
        threading.Thread(target=self._upload_worker, args=(path, url),
                         daemon=True).start()

    def _upload_worker(self, path: str, url: str):
        try:
            if path.endswith(".dat"):
                weights = load_dat(path)
            else:
                weights = load_pt(path)
    ...
    def _update_frame(self):
        ...
        self._cam_img.set_from_pixbuf(ndarray_to_pixbuf(preview_rgb))
        ...
        self._proc_img.set_from_pixbuf(ndarray_to_pixbuf(scaled_rgb))
    ...
    def _trigger_infer(self):
        ...
        threading.Thread(target=self._infer_worker,
                         args=(frame, url, invert), daemon=True).start()
        return True
    ...
    def _update_prediction(self, cls: int, lat_ms: float,
        ...
        self._pred_lbl.set_markup(
            f"<span size='xx-large'><b>Prediction: {name}</b></span>"
        )
        fps = self._compute_fps()
        self._stats_lbl.set_text(
    ...
    def _update_prediction_error(self, msg: str):
        self._pred_lbl.set_markup(
            "<span size='xx-large' foreground='red'><b>Error</b></span>"
        )
