"""Train CifarNet in FP32 on CIFAR-10 and export TorchScript for posit inference."""
import argparse
import random
import sys
...
def main():
    ...
    logger = EpochLogger(out / 'logs' / 'train_cifarnet_fp32.csv')
    best_acc = 0.0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), out / 'checkpoints' / 'ckpt_cifarnet_best.pt')

    torch.save(model.state_dict(), out / 'checkpoints' / 'ckpt_cifarnet.pt')
    export_jit(model, out / 'checkpoints' / 'ckpt_cifarnet_jit.pt')
    logger.close()

    save_meta(out / 'logs' / 'train_cifarnet_fp32_meta.json', {
        'run': 'cifarnet_fp32',
        'model': 'cifarnet',
        'format': 'fp32',
        'epochs': args.epochs,
