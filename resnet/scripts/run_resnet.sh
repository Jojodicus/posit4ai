...
CKPT_P8E2=$CKPT_DIR/ckpt_resnet18_p8e2_ft.dat
...
echo "=== ResNet: posit finetune p8e2 (quire) ==="
cpp/build_quire/finetune_posit \
    $MODEL $CKPT_FP32_JIT float $DATA 8 2 10 \
    $CKPT_P8E2 resnet18_p8e2_ft $LOG_DIR/finetune_p8e2.csv

echo "=== ResNet: posit inference p8e2_ft (quire) ==="
cpp/build_quire/inference_posit \
    $MODEL $CKPT_P8E2 posit $DATA 8 2 \
    resnet18_p8e2_ft $INF_CSV
...
echo "=== ResNet: posit inference p8e2_ft (no quire) ==="
cpp/build_noquire/inference_posit \
    $MODEL $CKPT_P8E2 posit $DATA 8 2 \
    resnet18_p8e2_ft $INF_CSV
