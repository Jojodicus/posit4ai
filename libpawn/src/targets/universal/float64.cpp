...
p::Number p::Float64Target::abs(p::Number a) { double v = toDouble(a); return toNumber(v < 0 ? -v : v); }
p::Number p::Float64Target::sqrt(p::Number a) { return toNumber(std::sqrt(toDouble(a))); }
p::Number p::Float64Target::relu(p::Number a) { double v = toDouble(a); return toNumber(v < 0 ? 0 : v); }
